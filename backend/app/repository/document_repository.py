...
def create_database_document(db: Session, document: schemas.DataBaseDocumentCreate):
    db_document = models.DataBaseDocument(id=document.id, contractor=document.contractor, contractorCNPJ=document.contractorCNPJ, hired=document.hired, hiredCNPJ=document.hiredCNPJ)
    db.add(db_document)
    db.commit()
